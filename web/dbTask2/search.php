...
<body>
  ...
  <div class="col-8">
    ...
    <form method="POST">
      <label>Search</label>
      <input type="text" name="search" value="<?php if (isset($_POST['search'])){print $_POST['search'];}?>" autofocus onfocus="let x=this.value; this.value=null; this.value=x" />
      <select name="select" onchange="checkSelect();" id="select">
        <option value="name" <?php if (isset($_POST['select'])){if ($_POST['select'] == "name") {
                                print "selected=\"selected\"";
                              }}?>>Name</option>
        <option value="cost" <?php if (isset($_POST['select'])){if ($_POST['select'] == "cost") {
                                print "selected=\"selected\"";
                              }} ?>>Cost</option>
        <option value="rating" <?php if (isset($_POST['select'])){if ($_POST['select'] == "rating") {
                                  print "selected=\"selected\"";
                                }} ?>>Rating</option>
      </select>
      <div id="radio">
        <label for="radio">&#60</label>
        <input type="radio" name="costSort" value="<" <?php if(isset($_POST['costSort'])){if ($_POST['costSort'] == "<" && $_POST['select'] == "cost") {
                                                        print "checked=\"checked\"";
                                                      }} ?> />
        <label for="radio">&#60&#61</label>
        <input type="radio" name="costSort" id="defaultCostSort" value="<=" <?php if(isset($_POST['costSort'])){if ($_POST['costSort'] == "<=") {
                                                                              print "checked=\"checked\"";
